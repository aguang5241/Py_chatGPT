...
class Super_Assistant(QMainWindow):
    def __init__(self):
        ...
        #     self.root_path = current_path.split('/Contents')[0]
        # else:
        #     self.root_path = os.path.join(current_path, 'Super_Assistant.app')
        self.ui.textBrowser.append(f"<p style='color:red'>Current path: {current_path}</p>")
    ...
    def speech_to_text(self):
        ...
        # try:
        #     user = r.recognize_google(audio, language=self.language_code)
        # except Exception as e:
        #     self.ui.textBrowser.append(f"<p style='color:red'>Error: {e}</p>")
        #     user = ''
    ...
    def get_text(self):
        ...
        # print(self.messages)
        # Call self.send_text() in a new thread using 
        # threading._start_new_thread(self.send_text, ())
        self.send_text_worker = Send_Text_Worker(self.messages)
        self.send_text_worker.signals.result.connect(self.send_text)
        self.threadpool.start(self.send_text_worker)
